...
const RoadmapCard = ({ icon, title, desc, color, delay }) => {
  const { ref, inView } = useInView(0.1);
  const [hovered, setHovered] = useState(false);
  return (
    <div
      ref={ref}
      className="group relative rounded-2xl overflow-hidden cursor-pointer border border-slate-200 bg-white shadow-sm hover:shadow-xl transition-all duration-300"
      style={{
        opacity: inView ? 1 : 0,
    ...
    >
      <div
        ...
        style={{ background: `linear-gradient(to right, ${color}, #8B5CF6)`, opacity: hovered ? 1 : 0.3 }}
      />
      <div className="relative p-6">
        <div
          className="absolute -top-10 -right-10 w-32 h-32 rounded-full blur-2xl transition-opacity duration-500"
          style={{ background: color, opacity: hovered ? 0.12 : 0.04 }}
        />
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="px-2 py-1 rounded-md text-[10px] font-bold bg-slate-100 text-slate-500 uppercase tracking-wider border border-slate-200">
              Coming Soon
            </span>
          </div>
          <h3 className="text-lg font-bold text-slate-800 mb-2 transition-colors duration-300" style={{ color: hovered ? color : '' }}>
            {title}
          </h3>
...
const Features = () => {
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 via-blue-50/30 to-purple-50/20 text-slate-800 overflow-x-hidden min-h-screen">
      ...
      {/* ── FUTURE ROADMAP ───────────────────────────────────────────────────── */}
      <section className="relative py-24 px-4 overflow-hidden bg-gradient-to-br from-slate-50 to-blue-50/30">
        <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-blue-200 to-transparent" />

        <div className="absolute top-1/4 right-0 w-96 h-96 bg-blue-200/30 rounded-full blur-[80px] pointer-events-none" />
        <div className="absolute bottom-0 left-0 w-96 h-96 bg-purple-200/20 rounded-full blur-[80px] pointer-events-none" />

        <div className="relative z-10 max-w-6xl mx-auto">
          <SectionHeader
            badge="Future Roadmap"
            title={<>The Future of Legal Tech<br /><span className="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Is Being Built Here</span></>}
