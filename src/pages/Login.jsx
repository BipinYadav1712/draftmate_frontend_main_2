...
const Login = () => {
    ...
    const getInputClasses = (valid, focused, typing) => {
        if (typing) return 'border-blue-400 ring-2 ring-blue-300/30 bg-blue-50/30 dark:bg-blue-900/10';
        if (valid === true) return 'border-emerald-500 ring-2 ring-emerald-400/25 bg-emerald-50/40 dark:bg-emerald-900/10';
        if (valid === false) return 'border-red-500 ring-2 ring-red-400/25 bg-red-50/40 dark:bg-red-900/10';
        if (focused) return 'border-blue-500 ring-2 ring-blue-400/25 bg-white dark:bg-slate-800';
    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-900 font-sans antialiased text-slate-900 dark:text-white h-screen overflow-hidden flex transition-colors duration-300">
            ...
            <div className="w-full lg:w-1/2 h-full flex flex-col bg-white dark:bg-slate-900 relative overflow-y-auto z-10 transition-colors duration-300">
                ...
                {/* Subtle glow blobs in background */}
                <div className="absolute top-[-80px] left-[-80px] w-64 h-64 rounded-full bg-blue-400/10 blur-3xl glow-blob pointer-events-none" />
                <div className="absolute bottom-[-60px] right-[-60px] w-48 h-48 rounded-full bg-purple-400/10 blur-3xl glow-blob pointer-events-none" style={{ animationDelay: '1.5s' }} />
                ...
                <div className="flex-1 flex flex-col justify-center px-8 lg:px-24 xl:px-32">
                    <div className={`max-w-md w-full mx-auto space-y-8 ${shake ? 'animate-shake' : ''}`}>
                        ...
                        <form className="space-y-5" onSubmit={handleLogin}>
                            ...
                            >
                                <label className="text-slate-900 dark:text-white text-sm font-medium leading-normal flex items-center gap-2">
                                    <span style={{ animation: emailFocused || emailTyping ? 'inputLabelFloat 0.3s ease-out forwards' : 'none' }}>
                                        Email or Username
                                    </span>
                                    ...
                                    {!emailTyping && emailValid === true && (
                                        <span className="inline-flex items-center justify-center w-5 h-5" style={{ animation: 'successPop 0.4s cubic-bezier(0.34,1.56,0.64,1) forwards' }}>
                                            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none">
                                                <circle cx="12" cy="12" r="11" fill="#10b981" />
                                                <path className="check-path" d="M6.5 12l4 4 7-7" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" fill="none" />
                                            </svg>
                                        </span>
                                    )}
                                    {!emailTyping && emailValid === false && (
                                        <span className="inline-flex items-center justify-center w-5 h-5" style={{ animation: 'errorWiggle 0.5s ease-in-out forwards' }}>
                                            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none">
                                                <circle cx="12" cy="12" r="11" fill="#ef4444" />
                                                <path className="cross-path" d="M8 8l8 8M16 8l-8 8" stroke="white" strokeWidth="2.5" strokeLinecap="round" />
                                            </svg>
                                        </span>
                                    )}
                                </label>

                                <div className={`relative ${emailFocused ? 'input-shimmer' : ''}`}>
                                    <input
                                    ...
                                    <div className={`absolute right-4 top-1/2 -translate-y-1/2 transition-all duration-300 ${emailFocused ? 'text-blue-500' : 'text-slate-300'}`}>
                                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                                            <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                                        </svg>
                                    </div>

                                    {/* Bottom accent line (animates on focus) */}
                                    <div
                                        className="absolute bottom-0 left-1/2 h-0.5 rounded-full transition-all duration-500 ease-out"
                                        style={{
                                            width: emailFocused ? '100%' : '0%',
                                            left: emailFocused ? '0' : '50%',
                                            background: emailValid === false ? '#ef4444' : emailValid === true ? '#10b981' : '#3b82f6',
                                        }}
                                    />
                                </div>
                            ...
                            >
                                <label className="text-slate-900 dark:text-white text-sm font-medium leading-normal flex items-center gap-2">
                                    ...
                                    {!passwordTyping && passwordValid === true && (
                                        <span className="inline-flex items-center justify-center w-5 h-5" style={{ animation: 'successPop 0.4s cubic-bezier(0.34,1.56,0.64,1) forwards' }}>
                                            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none">
                                                <circle cx="12" cy="12" r="11" fill="#10b981" />
                                                <path className="check-path" d="M6.5 12l4 4 7-7" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" fill="none" />
                                    ...
                                    {!passwordTyping && passwordValid === false && (
                                        <span className="inline-flex items-center justify-center w-5 h-5" style={{ animation: 'errorWiggle 0.5s ease-in-out forwards' }}>
                                            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none">
                                                <circle cx="12" cy="12" r="11" fill="#ef4444" />
                                                <path className="cross-path" d="M8 8l8 8M16 8l-8 8" stroke="white" strokeWidth="2.5" strokeLinecap="round" />
                                ...
                                <div className={`relative ${passwordFocused ? 'input-shimmer' : ''}`}>
                                    <input
                                    ...
                                    {/* Bottom accent line */}
                                    <div
                                        className="absolute bottom-0 h-0.5 rounded-full transition-all duration-500 ease-out"
                                        style={{
                                            width: passwordFocused ? '100%' : '0%',
                                            left: 0,
                                            background: passwordValid === false ? '#ef4444' : passwordValid === true ? '#10b981' : '#3b82f6',
                                        }}
                                    />
            ...
            <div className="hidden lg:flex w-1/2 relative bg-slate-900 items-center justify-center overflow-hidden">
                ...
                <div className="relative z-30 max-w-lg px-12 text-center text-white">
                    ...
                    {/* Trust indicators */}
                    <div className="flex items-center justify-center gap-8 animate-fade-up"
                        style={{ opacity: 0, animationDelay: '0.7s', animationFillMode: 'forwards' }}>
                        <div className="flex flex-col items-center gap-2 hover-lift p-4 rounded-xl bg-white/10 backdrop-blur-sm border border-white/10">
                            <span className="material-symbols-outlined text-[28px] text-blue-300">verified_user</span>
                            <span className="text-xs font-semibold uppercase tracking-wider text-slate-200">Bank-grade Security</span>
                        </div>
                        <div className="w-px h-12 bg-white/20 rounded-full" />
                        <div className="flex flex-col items-center gap-2 hover-lift p-4 rounded-xl bg-white/10 backdrop-blur-sm border border-white/10">
                            <span className="material-symbols-outlined text-[28px] text-emerald-300">cloud_done</span>
                            <span className="text-xs font-semibold uppercase tracking-wider text-slate-200">99.9% Uptime</span>
                        </div>
                    </div>
                </div>
            </div>
